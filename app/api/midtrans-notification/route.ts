...
export async function POST(req: Request) {
  try {
    ...
    if (newStatus !== "pending") {
      ...
      // Ekstrak ID internal Supabase
      const internalOrderId = midtransOrderId.replace("belibeli-trx-", "");

      const { error: updateError } = await supabase
        .from("orders")
        .update({ status: newStatus, transaction_details: notificationJson })
        .eq("id", internalOrderId);

      if (updateError) {
        console.error("Supabase update error:", updateError);
        // Kirim 500 agar Midtrans mencoba lagi
    ...
    // Ini menghentikan Midtrans dari pengiriman notifikasi berulang
